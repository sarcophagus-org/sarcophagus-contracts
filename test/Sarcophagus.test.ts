  ...
  describe("registers an archaeologist", () => {
    describe("requires a public key", () => {
      describe("and should fail with a wrong public key length", () => {
        it("very short", async () => {
          ...
          await expect(sarco.callStatic.register(smallest, wallet2.address, 0, 0, 0)).to.be.revertedWith("public key must be 64 bytes")
        })

        it("slightly short", async () => {
          const smaller = pubKey(wallet).subarray(0, pubKey(wallet).length - 1)
          expect(smaller).to.have.lengthOf(63)
          await expect(sarco.callStatic.register(smaller, wallet2.address, 0, 0, 0)).to.be.revertedWith("public key must be 64 bytes")
        })

        it("slightly long", async () => {
          const bigger = Buffer.concat([pubKey(wallet), Buffer.from("00", "hex")])
          expect(bigger).to.have.lengthOf(65)
          await expect(sarco.callStatic.register(bigger, wallet2.address, 0, 0, 0)).to.be.revertedWith("public key must be 64 bytes")
        })

        it("very long", async () => {
          const biggest = Buffer.concat([pubKey(wallet), pubKey(wallet)])
          expect(biggest).to.have.lengthOf(128)
          await expect(sarco.callStatic.register(biggest, wallet2.address, 0, 0, 0)).to.be.revertedWith("public key must be 64 bytes")
        })
      })

      describe("and should succeed with a correct public key length", () => {
        it("just right", async () => {
          const register = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 0)
          expect(register).to.equal(true)
          await sarco.register(pubKey(wallet), wallet2.address, 0, 0, 0)
    ...
    describe("requires the archaeologist transaction initiator and public key match", () => {
      it("fails when the public key does not derive the transaction's msg.sender", async () => {
        ...
        await expect(sarco.callStatic.register(pubKey(wallet2), wallet.address, 0, 0, 0)).to.be.revertedWith("transaction address must have been derived from public key input")
      })
    ...
    describe("requires a minimum bounty value", () => {
      it("should succeed with a zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 0)
        expect(ret).to.be.true
      })

      it("should succeed with a non-zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 1, 0, 0)
        expect(ret).to.be.true
      })
    })
    ...
    describe("requires a minimum digging fee", () => {
      ...
      it("should succeed with a non-zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 1, 0)
        expect(ret).to.be.true
      })
    })
    ...
    describe("requires a maximum resurrection time", () => {
      ...
      it("should succeed with a non-zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 1)
        expect(ret).to.be.true
      })
    })
    ...
    describe("requires a bond", () => {
      it("should succeed with a zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 0, { value: 0 })
        expect(ret).to.be.true
      })

      it("should succeed with a non-zero value", async () => {
        const ret = await sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 0, { value: 1 })
        expect(ret).to.be.true
      })
    })
    ...
    describe("does not allow same key to register twice", () => {
      it("with with same payment address", async () => {
        await sarco.register(pubKey(wallet), wallet2.address, 0, 0, 0)
        await expect(sarco.callStatic.register(pubKey(wallet), wallet2.address, 0, 0, 0)).to.be.revertedWith("archaeologist already registered")
      })

      it("with different payment addresses", async () => {
        await sarco.register(pubKey(wallet), wallet2.address, 0, 0, 0)
        await expect(sarco.callStatic.register(pubKey(wallet), wallet.address, 0, 0, 0)).to.be.revertedWith("archaeologist already registered")
      })
    })
    ...
    describe("allows different keys to register", () => {
      it("with same payment address", async () => {
        ...
        await sarco.register(pubKey(wallet2), wallet.address, 0, 0, 0)
        const count = await sarco.archaeologistCount()
      ...
      it("with different payment addresss", async () => {
        await sarco.register(pubKey(wallet), wallet.address, 0, 0, 0)
        sarco = sarco.connect(wallet2)
        await sarco.register(pubKey(wallet2), wallet2.address, 0, 0, 0)
        const count = await sarco.archaeologistCount()
        expect(count).to.equal(2)
      })
    })
    ...
    describe("accumulates the contract value as archaeologists post their bond", () => {
      ...
      it("adds to the balance when an archaeologist registers", async () => {
        const ogBalance = await provider.getBalance(sarco.address)
        const bond = 1
        await sarco.register(pubKey(wallet), wallet.address, 0, 0, 0, { value: bond })
        const newBalance = await provider.getBalance(sarco.address)
      ...
      it("adds to the balance when multiple archaeologists register", async () => {
        ...
        const bond1 = 1
        const bond2 = 2
        await sarco.register(pubKey(wallet), wallet.address, 0, 0, 0, { value: bond1 })
        sarco = sarco.connect(wallet2)
        await sarco.register(pubKey(wallet2), wallet2.address, 0, 0, 0, { value: bond2 })
        const newBalance = await provider.getBalance(sarco.address)
