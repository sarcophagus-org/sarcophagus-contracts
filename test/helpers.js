...
const { ecsign, keccakFromHexString } = require("ethereumjs-util")
...
const generateKeys = () => {
  ...
  return { private: privateKeyBuffer, public: publicKeyBytes }
}

const deriveAddress = publicKey => {
  const wallet = Wallet.default.fromPublicKey(Buffer.from("04" + publicKey.toString("hex"), "hex"), true)
  const address = wallet.getChecksumAddressString()
  return address
}

const retrievePrivateKey = address => {
  // if this breaks, is truffle still using "candy maple cake ..."?
  const mapping = {
...
const signPubKeyAndAssetId = (pubKey, assetId, from) => {
  const privateKey = retrievePrivateKey(from)
  const privateKeyBuffer = Buffer.from(privateKey, "hex")
  const assetIdHex = Buffer.from(assetId).toString("hex")
  const dataHex = "0x" + pubKey.toString("hex") + assetIdHex
  const dataHash = keccakFromHexString(dataHex)
  const signature = ecsign(dataHash, privateKeyBuffer)
  return signature
}

module.exports = { generateKeys, deriveAddress, signPubKeyAndAssetId }
